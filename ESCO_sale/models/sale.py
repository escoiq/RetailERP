...
class sale_order(models.Model):
    ...
    def action_confirm(self):
        print (">>>>>>>>>>",self.env.user.company_id.force_restrict_sale_stock)
        if self.env.user.company_id.force_restrict_sale_stock:
            for order in self:
                for line in order.order_line:
                    if line.product_id and line.product_id.type == 'product':
                        draft_qty = sum([ x.product_uom_qty for x in self.env['sale.order.line'].search([('product_id','=',line.product_id.id),
                                                                        ('state','in', ['draft','sent']),
                                                                        ('id','!=',line.id) ]) ])
                        actual_available_qty = line.product_id.qty_available - draft_qty
                        print ("available = ",line.product_id.qty_available, '- draft_qty = ',draft_qty)
                        print ("actual after deducting draft",actual_available_qty)
                        print ("current demand",line.product_uom_qty)
